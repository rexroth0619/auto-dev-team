...
- Web 场景禁止使用 headless；必须真实拉起可见浏览器窗口，且不得随意改成后台执行。
...
- Playwright 必须固定 `headless: false`
- Web GUI 验证必须真实出现浏览器窗口；⛔ 禁止为了省事、提速或规避环境问题而改成 `headless: true`
- 若环境无法展示浏览器，默认判定为 `Manual only / 暂不可执行`，而不是静默退化为 headless
...
- `required`：默认值；浏览器窗口必须真实出现
- `unavailable`：当前环境无法把 GUI 过程直接展示给用户

规则：

- Web GUI 默认 `visual_mode=required`，且必须 headed 执行。
- 禁止把 `required` 悄悄改成后台 headless 运行。
- 若只能后台运行，必须把 `visual_mode` 标为 `unavailable`，并明确转 `Manual only / 暂不可执行`，同时补足截图、trace、timeline 等可回放证据。
...
2. 选择 executor 与 `visual_mode`；Web GUI 固定 headed，浏览器窗口必须出现。
...
```text
🖥️ 前端GUI测试开始 - [GUI-{任务指纹}-{Step|Feature}-{caseID}-{executor}-r{轮次}] {scope=主验证/supplemental | visual=headed-required | gate=GUI}
```
