...
> 适用: 想写测试、补测试、验证某个 use case、检查覆盖缺口、做预发验收、根据最近提交生成测试数据和手测步骤 | 产出: 测试文件；命中大测试时再产出 `current-test.md`；需要发版验收时可产出预发验收包
...
5. 等用户贴回查询结果后，再生成更准确的测试数据单
6. 基于测试数据，列出最相关的可测 use cases 和手测步骤
```
...
- 若需要沉淀为文档，优先使用 `assets/templates/release-test-pack.md` 生成 `.autodev/temp/release-test-pack.md`
- 若命中大测试，预发验收包之外仍需同步维护 `.autodev/current-test.md`
...
✅ 预发验收包已生成
- 汇总查询语句数量、测试数据单数量、可测 use case 数量和下一步建议
```
...
| 想根据最近提交准备预发验收 | 先提炼行为变化；不确定则先输出查数 SQL，再生成测试数据单和手测步骤 |
