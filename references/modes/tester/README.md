...
> 适用: 想写测试、补测试、验证某个 use case、检查覆盖缺口、做预发验收、根据最近提交生成测试计划 | 产出: 测试文件；命中大测试时再产出 `current-test.md`；需要发版验收时先生成 `release-plan.json`，再分流到手动或自动执行
...
```text
AI 在“预发验收 / 发版手测 / 根据最近提交生成测试计划”场景下，执行以下流程:

1. 读取最近提交、用户指定提交范围或 PR diff
2. 若命中自动化预发，先读取 `.autodev/ai-sot.json`
3. 只提炼对测试有影响的行为变化，不复述全部 diff
4. 生成 `.autodev/temp/release-plan.json`
5. 先返回一条简洁回执，再让用户选择 `手动 / 自动`
6. 选 `手动` -> 保留当前人工预发流程
7. 选 `自动` -> 读取 `references/principles/release-auto-loop.md`
```

### 自动模式前置检查表（强制）

进入自动模式前，AI 必须先显式确认并在用户可见回执中体现：

- `后端执行宿主`：已确认 / 未确认
- `后端 SSH 接入方式`：`alias_only` / `jump_alias` / `web_terminal_only`
- `GUI 执行宿主`：`local_browser` / `local_runner` / `remote_browser` / `manual_fallback`
- `认证策略`：`existing_session` / `browser_login_handoff` / `local_secret_store` / `manual_fallback`
- `GUI 执行`：已配置 / 未配置
- `写操作权限`：允许 / 不允许

⛔ 若以上任一项未明确，禁止宣称“开始自动化预发测试”。
⛔ 若用户明确要“自动化预发测试”且目标包含 GUI，不得 silently 降级成后端-only 自动检查。
⛔ 若后端 SSH 仅能通过云厂商网页终端（`web_terminal_only`）访问，默认直接 `manual_fallback`。
⛔ SSH 推荐输入必须是 `ssh alias` 或 `jump alias`，不得要求用户在聊天里提供密码、私钥内容或完整敏感 SSH 凭证。
⛔ GUI 默认优先本机执行；除非用户明确要求，否则不得把 GUI 执行绑定到远端 SSH 宿主。
...
- `release-plan.json` 是自动化版唯一的机器计划输入，不再默认生成长 markdown 草稿
- `.autodev/ai-sot.json` 是自动化版项目固定事实真相源；AI 默认禁止修改，除非用户明确确认
- 手动模式继续由 LLM 直接返回简洁回执、查询建议、use cases 和手测步骤
- 自动模式必须先有 `release-plan.json`，没有 plan 不得执行
- 自动模式必须先有 `.autodev/ai-sot.json`；没有 `ai-sot` 不得执行
- 自动模式默认 fail-close；缺少认证态、查询入口、造单入口、必需命令时立即转人工
- 若任务命中 `UI / GUI` 域，`release-plan.json` 中必须存在高优先级 GUI checks；否则直接 `manual_fallback`
- 若自动模式缺少后端 SSH alias / jump alias / 远端工作目录 / `allowed_paths`，不得宣称进入“自动化预发测试开始”
- 若 plan 的命令越出 `allowed_paths`，必须直接 fail-close
- 自动模式需要写操作时，必须显式放行
- 若命中大测试，预发测试链路之外仍需同步维护 `.autodev/current-test.md`

### 执行模式分流

```text
手动:
- 保留当前“查数 -> 造单 -> use cases -> 手测步骤”的交互式流程

自动:
- 读取 `references/principles/release-auto-loop.md`
- 固定顺序: summary -> cleanup -> auth -> query -> seed -> be -> gui -> evidence -> receipt
- 启动前先用 emoji 回执明确:
  - 是否有后端测试
  - 是否有前端 GUI 测试
  - executive summary
