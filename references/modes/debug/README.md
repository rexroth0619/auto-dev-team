...
> 不要头疼医头、脚疼医脚。一个症状背后，可能站着一类缺陷、一个重复实现、或一段平台级机制缺口。

确诊后，默认必须继续问三件事：

1. 这是不是某一类 bug，而不只是这一个点坏了？
2. 其他类似模块 / 对称路径 / 平行实现里，会不会有同型问题？
3. 这次应该做单点修补，还是做平台级 / 机制级修复，才能避免同类问题再次出现？

**⛔ 禁止只把当前报错点修到“不报错了”，却不检查同类实现和系统性缺口。**
...
- 假设不只描述“这个点为什么坏”，还要判断它属于哪一类缺陷：
  ...
  - 平台级封装缺失
  - 兼容旧调用 / 旧数据问题
- 若怀疑属于“同类模式问题”，后续治疗方案必须包含扩散扫描，不得只修首个命中点
...
医师: 1. 先组装最小场景矩阵：
         - 原始复现路径
         - 至少 1 个负例 / 边界 case
         - 复杂问题或多轮排查时，初始化 / 更新 `.autodev/current-debug.md`，并保持 flow metadata 与 active flow 一致
...
医师: 1. "🔬 确诊: xxx"
      ...
      4. ⭐ 按 `references/principles/critique.md` 的模式集成流程执行自动会诊
         传递：【用户原始问题描述】+ 确诊结论、治疗方案、依据
      5. 等待用户选择（禁止先执行）
      6. 💿 执行前快照闸门（强制）
         - 必须输出 "💿 已保护" 或 "💿 闸门通过" 后才能继续
      6.5 🧭 治疗前 Blast Radius（强制）
         - 对计划修改的文件 / 符号执行 `scripts/blast-radius.py ... --mode debug --write`
         - 更新 `.autodev/current-blast-radius.md`
         - 若影响面超出当前治疗方案，先停下缩小手术范围
      7. 确认后执行，观察治疗效果（复用同一组观测场景；GUI 问题则复用同一组 GUI case + evidence bundle）
      8. 清理所有临时 [DEBUG-*] 诊断探针，仅保留必要基础观测
```
...
2. 即时验证（主 Agent）
   - 原始复现路径必须通过
   - 若本次做了同类模块批量修复或平台级修复，至少抽样验证 1-2 个同型入口
   - 若存在对称路径，至少验证 1 个对称操作
    ↓
3. 输出给 PM
```
...
- **先分类再治疗**: 确诊后，必须先判断这是单点 bug 还是某一类缺陷
- **先扩散检查再定层级**: 默认检查类似模块 / 平行实现 / 对称路径，评估是否存在同型问题
- **优先合适层级修复**: 能用小而稳的平台级抽象解决同类问题时，不得只打单点补丁；若仍选择单点修复，必须说明理由
...
- **防复发回归**: 若本次修复覆盖一类问题，必须补相应回归保护，避免别处再炸
