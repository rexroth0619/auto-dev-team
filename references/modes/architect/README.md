...
- 若存在 `.autodev/current-stack.json`，同步读取当前活跃栈
- `.autodev/project-map.md` - 了解项目结构
- `.autodev/module-registry.md` - 查找可复用模块 / 组件 / 适配器
- `references/principles/code-navigation.md` - 查现有模块、入口、复用点、调用链和候选影响面
...
进入 Architect 前，必须已经完成 `references/shared/interaction-contract.md` 的 `需求确认与澄清闸门`。若用户尚未回复澄清问题，或目标仍有歧义，先停在 Phase 0 或切到 Brainstorm，不得直接进入计划或 FastTrack。
...
AI:   0. 主动读取 project-map.md 和 module-registry.md
      1. 苏格拉底式提问（按轮次一次性给出当前阶段全部高信号问题）
         - 目的: 你要解决什么问题？
         - 用户: 谁会用？什么场景下用？
         - 边界: 有什么限制？（时间 / 技术栈 / 兼容性）
         - 成功: 做成什么样算完成？
      2. 优先按 `Brainstorm 问题包` 分组输出，让用户一次性填写
      3. 若本轮回答后仍有缺口，再给下一轮补充问题包；不要退回连续单问模式
      4. 需求足够清晰后 → 进入 Phase 1
```

⛔ 禁止把本轮可一次性收集完的问题拆成多条消息逐个追问
⛔ 先问目的，再问方案
...
AI:   0. 测试环境检查:
      ...
      1. 查 module-registry: "搜索 [关键词]: 找到 xxx / 未找到"
         若代码导航器可用且项目已索引，同时执行 semantic_explore / semantic_search，确认现有入口、复用点、调用链和相邻测试，避免重复新建模块。

      1.2 Scope Router（V2）:
         - 依据目标数量、影响面、不确定性、依赖、验证复杂度、持续时长、AI把握度
         - 判断当前最小安全规划层级：
           `one-shot / flow / phase / milestone / project`
         - 若判断结果高于 flow：
           - 优先生成更高层规划 artefact（如 `project-roadmap.md`）
           - 只细化当前活跃前沿，不强制一步拆到底

      1.5 🏠 归属判断（强制，在设计新结构前）:
          - 先按术语锁把“组件 / 服务 / API / 边界 / 抽象”等模糊词翻译成模块、接口面、接缝、适配器或测试面
          ...
          ├── [融入] 扩展 {现有模块}（新增哪个接口面 / 实现行为）
          ├── [复用] 调用 {现有模块/工具}（列出接口面和契约）
          ├── [接缝] 引入或移动 {接缝}（列出适配器与理由）
          └── [新建] 创建 {新模块}（理由、主接口面、职责）

      2. 影响范围分析: 🟢小 / 🟡中 / 🔴大
         - 先用代码导航器收窄目标文件 / 符号 / 入口
         - 再执行 `scripts/blast-radius.py` 生成候选影响面
         - 调用方 / 消费方 / 契约兼容性
         - 地址 / 路径 / 配置 / 数据格式变化
      ...
      5. 🔷 抽象潜力评估（三问法则）
         - 抽象建议必须用术语锁说清：是加深模块、收窄接口面、移动接缝、引入适配器，还是只清理实现
      6. 判断: 💗更优方案 / ⚠️有风险 / ✅可行
```
...
AI:   1. 评估复杂度:
         ...
         - [中等] 4-5步
         - [复杂] >5步 → 建议拆分需求
         - 若当前层级判断为 `phase / milestone / project`：
           - 先产出对应上层规划，再为当前活跃 flow 生成 `current-steps.md`
...
如果改动很小（≤2 文件，≤30 行）→ 在需求确认和澄清问题包均已完成后，跳过完整计划，自动切换到 FastTrack 模式并输出快速方案 + 会诊；不得绕过 `🧾 需求确认` 或 `🧾 需求澄清问题包` 直接一键实现。

## 强制检查

- 单文件职责过多或体积明显膨胀 → 建议拆分，提醒用户
- 发现可复用模块 / 组件 / 适配器 → 必须复用，不得新建
