...
- `references/principles/code-navigation.md` - 查性能路径、调用链、复用点、相邻测试和候选影响面
- 用户提到的目标文件

## Phase 0: 需求确认与澄清

进入 Optimize 前，必须先完成 `references/shared/interaction-contract.md` 的 `需求确认与澄清闸门`。

- 先输出 `🧾 需求确认`
- 用户确认后，输出 `🧾 需求澄清问题包`，重点问“什么慢、慢到什么程度、影响哪些场景、优化验收指标”
- 输出澄清问题包后必须停下来等用户回复；用户回复前不得做性能诊断、方案或代码改动
...
AI:   1. 明确: 什么慢? 慢到什么程度? 影响哪些场景?
      ...
      2. 🔍 诊断清单 (Show Your Work):
         | 类型 | 检查项 | 状态 | 发现 |
         | 计算/渲染/网络/内存 | ... | ✅/❌ | ... |
         - 若代码导航器可用且项目已索引，先用 semantic_explore 定位性能路径、热点函数候选、调用链和相邻测试
         
      3. 影响范围: 🟢小 / 🟡中 / 🔴大
         - 单点优化也先用代码导航器收窄目标，再跑 `scripts/blast-radius.py`
```
