...
**过度设计的特征**（出现任何一条就是过度设计）：
- ❌ 为"未来可能需要"的场景预留扩展点
- ❌ 引入 PM 没提到的抽象层（Factory、Strategy、Observer...）
- ❌ 创建只被调用一次的 util/helper 函数
- ❌ 为单一实现定义 interface/抽象类
- ❌ 添加 PM 没要求的配置项、参数、开关
- ❌ 使用复杂设计模式解决简单问题
...
- ✅ 只解决 PM 明确提出的需求
- ✅ 用最直接的方式实现，不绕弯
- ✅ 代码重复 2 次没关系，3 次再考虑抽象
- ✅ 宁可代码"笨"一点，也不要"聪明"得难以理解
- ✅ 有疑问时选择更简单的方案

**自检问题**（写代码前必须问自己）：
1. 这个抽象是 PM 要求的，还是我觉得"应该有"的？
2. 如果删掉这层封装，功能还能正常工作吗？
3. 一个不懂代码的人能理解这个结构吗？

**如果答案是"我觉得应该有"或"删掉也行"→ 不要加。**
