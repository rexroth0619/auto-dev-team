...
**`.autodev/` 存放在项目的上一层目录**，命名为 `.autodev-{项目文件夹名}/`。

例如：项目路径 `/path/to/ai_master/` → 文档位于 `/path/to/.autodev-ai_master/`

**⛔ 禁止修改项目本身的 `.gitignore`**——这样可避免团队协作冲突。
...
> 注：文档中 `.autodev/` 为简写，实际路径按"存放位置"规则。
...
📌 是否沉淀为通用 Pattern？
[1] 沉淀到 skill 本体（进入 auto-dev-team/pattern-write）→ 写入 references/patterns/
